...
- Меняется верхняя строка преимуществ: *«ТОО «KazTextileА» • Официальный НДС 12% (ОУР) • ИС ЭСФ • КС-2/КС-3 за 24ч • Сертификаты КМ1 МЧС РК»*.
...
| **Отели, Апартаменты, HoReCa** | Строгие пожарные нормы МЧС РК, порча штор гостями, частые химчистки, необходимость полной темноты для сна гостей. | Контрактный негорючий текстиль Trevira CS (КМ1) и блэкаут плотностью от 280 г/м². Устойчивость к стирке при 60°C, защита от усадки и потери противопожарных свойств. | Пожарные сертификаты РК (ТР ТС 043/2017, СНиП РК 2.02-05-2009), протоколы испытаний на истираемость по Мартиндейлу (>50 000 циклов). |
...
| **Девелоперы и Генподрядчики (ЖК, БЦ, ТЦ)** | Соблюдение графика сдачи объекта, согласование смет, бюрократия, формы КС-2/КС-3, риск срыва госкомиссии. | Комплексное закрытие объемов солнцезащиты «под ключ»: поставка, монтаж бригадами с допусками, исполнительная документация, закрытие через КС-2/КС-3 с НДС 12%. | Фиксация цен в договоре в тенге ₸, штрафные санкции за каждый день просрочки, прозрачные сметы ТОО «KazTextileА». |
...
- **Нормативное соответствие**:
  ...
  - Класс пожарной опасности материала: **КМ1** (разрешено для применения в путях эвакуации, холлах, ресторанах, отелях, залах свыше 50 человек и госучреждениях РК).
...
## 3. Специфика работы с юрлицами (Corporate Trust & Workflow ТОО «KazTextileА»)

```mermaid
flowchart TD
    A["Получение ТЗ / Ведомости проемов"] -->|60–120 мин| B["Экспресс-КП с НДС 12% и разбивкой по зонам"]
    B --> C["Выезд инженера с кейсом образцов на объект в Астане"]
    C --> D["Лазерный замер и фиксация кабельных выводов"]
    D --> E["Официальный договор ТОО «KazTextileА» (ОУР, НДС 12%)"]
    E --> F["Производство в цехе 350 м² в Астане + стендовый тест моторов"]
    F --> G["Монтаж сертифицированной бригадой (пылеудаление Hilti)"]
    G --> H["Закрытие: ИС ЭСФ / Акт АВР (Р-1) / КС-2 и КС-3 + Паспорта + Сертификаты МЧС КМ1"]
```

### 3.1. Финансово-юридический стандарт
1. **Расчет с официальным НДС 12%**:
   - ТОО «KazTextileА» (БИН 140940019744, г. Астана) на Общеустановленном режиме налогообложения (ОУР). Входящий НДС 12% в полном объеме принимается к вычету бухгалтерией корпоративного заказчика в РК.
2. **Экспресс-КП за 60–120 минут**:
   - При отправке дизайн-проекта, ведомости отделки, планировки или чертежей в форматах PDF / Excel / DWG менеджер проекта готовит детальный расчет сметы за 1–2 часа.
   - Спецификация формируется с разбивкой по каждому помещению (наименование, размеры, тип управления, модель ткани/ламелей, единичные расценки, монтаж, НДС 12%).
3. **Мобильный шоурум на объекте**:
   - Выезд ведущего инженера прямо на стройплощадку или в офис заказчика в Астане с профессиональным кейсом:
     - Рабочий стенд мотора Somfy Sonesse / Forest с радиопультом;
     - Натуральные образцы валов и кассетных профилей в разрезе;
     - Веера негорючих тканей Trevira CS, образцов Screen (1%, 3%, 5%) и ламелей жалюзи 50 мм по каталогу RAL.
4. **Полный пакет закрывающих документов за 24 часа**:
   - Выписка ЭСФ через государственную информационную систему **ИС ЭСФ**.
   - Акт выполненных работ / оказанных услуг (**форма Р-1**).
   - Для монтажных и строительных работ: Акт приемки выполненных строительно-монтажных работ (**форма КС-2**) и Справка о стоимости работ и затрат (**форма КС-3**).
   - Паспорта на изделия с индивидуальными серийными номерами приводов и заверенные копии пожарных сертификатов соответствия МЧС РК (класс КМ1).
5. **Условия расчетов**:
   - Аванс 50–70% на запуск в производство, остаток по факту подписания КС-2 / АВР (Р-1) и выставления ЭСФ.
   - Фиксация сметной стоимости строго в национальной валюте тенге (₸).
   - Для проверенных генподрядчиков и постоянных корпоративных партнеров предусмотрена отсрочка платежа до 15 банковских дней.
